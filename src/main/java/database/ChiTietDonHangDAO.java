...
public class ChiTietDonHangDAO implements DAOInterface<ChiTietDonHang> {
	...
	public ArrayList<ChiTietDonHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				DonHang dh = new DonHangDAO().selectById(new DonHang(donhang, null, sql, donhang, maChiTietDonHang, donhang, sanpham, thuevat, tongtien, null, null));
	...
	public ChiTietDonHang selectById(ChiTietDonHang t) {
		...
		try {
			...
			String sql = "SELECT * FROM chitiet WHERE machitietdonhang=?";
			PreparedStatement st = con.prepareStatement(sql);
	...
	public int insert(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "INSERT INTO chitiet (machitietdonhang, donhang,sanpham, soluong, giagoc,giamgia,giaban,thuevat,tongtien) "
					+ " VALUES (?,?,?,?,?,?,?,?,?)";
	...
	public int delete(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "DELETE from chitiet " + " WHERE machitietdonhang=?";
	...
	public int update(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE chitiet SET donhang=?, sanpham=?, soluong=?, giagoc=?, giamgia=?, giaban=?, thuevat=?, tongtien=?"
